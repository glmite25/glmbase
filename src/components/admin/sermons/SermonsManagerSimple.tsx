...
import { Plus, Edit, Trash2, Play, Download, Eye, Calendar, User, Clock, BookOpen } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const SermonsManagerSimple = () => {
  ...
  const fetchSermons = async () => {
    try {
      ...
      // Use raw SQL query since sermons table might not be in types yet
      const { data, error } = await supabase
        .rpc('get_sermons_with_pastors');

      if (error) {
        console.warn('RPC call failed, trying direct query:', error);
      ...
      // If sermons table doesn't exist, show placeholder
      setSermons([]);
      toast({
        title: "Info",
        description: "Sermons table not yet set up. Please run the database setup script.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const fetchPastors = async () => {
    try {
      ...
      // Set default pastor if query fails
      setPastors([{ id: 'default', name: 'Pastor Lawrence Ojide' }]);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to save sermon. Please ensure the database is set up correctly.",
      });
    }
  };
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{sermons.filter(s => s.status === 'published').length}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{sermons.filter(s => s.is_featured).length}</p>
              </div>
          ...
          <CardContent className="p-4">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{sermons.reduce((sum, s) => sum + s.view_count, 0)}</p>
              </div>
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <p className="text-gray-600 mb-4">
              Get started by adding your first sermon or run the database setup to create the sermons table.
            </p>
              ...
              <CardContent className="space-y-3">
                <div className="text-sm text-gray-600 space-y-1">
                  <div className="flex items-center">
                    ...
                    {sermon.pastor_name || 'Unknown Pastor'}
                  </div>
                  <div className="flex items-center">
...
export default SermonsManagerSimple;