-- IMPLEMENT DATABASE RECOMMENDATIONS - SAFE VERSION
-- This version avoids constraint conflicts by being more permissive
...
-- ========================================
-- PART 2: BASIC DATA VALIDATION CONSTRAINTS
-- ========================================

-- Email validation constraint (only if email is provided)
...
-- Phone validation constraint (very permissive - just check it's not empty if provided)
...
-- Category validation constraint (only validate known categories)
...
ALTER TABLE public.members ADD CONSTRAINT valid_category 
    CHECK (category IS NULL OR category IN ('Pastors', 'Members', 'MINT'));

-- Church unit validation constraint (very permissive - just not empty if provided)
...
SELECT 'Basic validation constraints created successfully!' as status;
...
-- Function to prevent users from changing restricted fields
CREATE OR REPLACE FUNCTION public.prevent_restricted_field_changes()
RETURNS TRIGGER AS $$
BEGIN
    -- Only apply restrictions if this is not a superuser
    IF NOT EXISTS (
        SELECT 1 FROM public.user_roles ur 
        WHERE ur.user_id = auth.uid() 
        AND ur.role = 'superuser'
    ) THEN
        -- If user is updating their own record, prevent changing restricted fields
        IF OLD.user_id = auth.uid() THEN
            NEW.category = OLD.category;        -- Can't change their own category
            NEW.user_id = OLD.user_id;         -- Can't change user_id
            NEW.assignedto = OLD.assignedto;   -- Can't change assigned pastor
            NEW.churchunit = OLD.churchunit;   -- Can't change church unit
            NEW.isactive = OLD.isactive;       -- Can't change active status
        END IF;
    END IF;
...
DROP TRIGGER IF EXISTS prevent_restricted_changes ON public.members;
CREATE TRIGGER prevent_restricted_changes
    BEFORE UPDATE ON public.members
    FOR EACH ROW EXECUTE FUNCTION public.prevent_restricted_field_changes();

SELECT 'Audit trail and field restriction functionality implemented successfully!' as status;
...
SELECT 'Row Level Security policies implemented successfully!' as status;
...
SELECT 
    COALESCE(churchunit, 'Unassigned') as churchunit,
...
SELECT 'Church unit statistics view created successfully!' as status;
...
SELECT 'DATABASE RECOMMENDATIONS IMPLEMENTATION COMPLETED SUCCESSFULLY!' as status;

-- Show church unit distribution
...
-- Show RLS policies
SELECT 'Security Policies Created:' as info;
SELECT 
    tablename,
    policyname
FROM pg_policies 
...
SELECT '✅ All database recommendations implemented successfully!' as final_status;
SELECT '✅ Your database is now optimized with indexes, security, and validation!' as final_message;