...
END $$;-- Create
 trigram indexes if extension is available
DO $$
BEGIN
...
BEGIN
    ...
    SELECT 
        ...
        CASE 
            ...
            ELSE 'WARNING'::TEXT
        END,
        'Profiles table has ' || profiles_index_count::TEXT || ' indexes'::TEXT;    -- V
alidation 2: Critical indexes exist
    RETURN QUERY
    SELECT 
...
BEGIN
    ...
    RETURN;
END;
$$ LANGUAGE plpgsql;GRA
NT EXECUTE ON FUNCTION public.test_enhanced_query_performance() TO authenticated;
